...
func (gq *GroupQuery) QueryTabs() *TabQuery {
	query := &TabQuery{config: gq.config}
...
func (gq *GroupQuery) QueryClient() *ExtensionClientQuery {
	query := &ExtensionClientQuery{config: gq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := gq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (gq *GroupQuery) Clone() *GroupQuery {
	...
	return &GroupQuery{
		...
		withTabs:   gq.withTabs.Clone(),
		withClient: gq.withClient.Clone(),
...
func (gq *GroupQuery) WithTabs(opts ...func(*TabQuery)) *GroupQuery {
	query := &TabQuery{config: gq.config}
...
func (gq *GroupQuery) WithClient(opts ...func(*ExtensionClientQuery)) *GroupQuery {
	query := &ExtensionClientQuery{config: gq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (gq *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := gq.withTabs; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Group)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Tabs = []*Tab{}
		}
		query.withFKs = true
		query.Where(predicate.Tab(func(s *sql.Selector) {
			s.Where(sql.InValues(group.TabsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.group_tabs
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "group_tabs" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "group_tabs" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Tabs = append(node.Edges.Tabs, n)
		}
	}

	if query := gq.withClient; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Group)
		for i := range nodes {
			if nodes[i].extension_client_groups == nil {
				continue
			}
			fk := *nodes[i].extension_client_groups
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "extension_client_groups" returned %v`, n.ID)
			}
			for i := range nodes {
