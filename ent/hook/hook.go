...
func (f ExtensionClientFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.ExtensionClientMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.ExtensionClientMutation", m)
	}
	return f(ctx, mv)
}
